...
template<typename T>
class random_pyramidic : public Player<T> {
public: // Change this from protected/private to public
    random_pyramidic(T symbol) : Player<T>("Computer", symbol) {}
    T get_board_value(int x, int y) const;

    void getmove(int& x, int& y) override {
        do {
            x = rand() % 3;            // Row within pyramid structure
            y = rand() % (2 * x + 1); // Column within pyramid width
        } while (this->get_board_value(x,y)!='*');
        cout << "Computer placed " << this->symbol << " at (" << x << ", " << y << ")\n";
    }
};
template <typename T>
myboard<T>::myboard() {
...
template<typename T>
void PyramidicPlayer<T>::getmove(int &x, int &y) {
     cout << this->name << ", enter your move (row and column starting from 0)" ;
            cin >> x >> y;
}
template <typename T>
T random_pyramidic<T>::get_board_value(int x, int y) const {
    return this->get_board_value(x,y);
}
 */
template <typename T>
class myboard : public Board<T> {
